...
d1 = fetch_20newsgroups(subset='train', shuffle=True, random_state=1, remove=('From', 'headers', 'footers', 'quotes'))
d2 = fetch_20newsgroups(subset='test', shuffle=True, random_state=1, remove=('From', 'headers', 'footers', 'quotes'))
...
new_string = ' '.join([w for w in fd[1].split() if len(w)>2])
print(new_string)
...
for i in range(0, len(fd)):
    new_string = ' '.join([w for w in fd[i].split() if len(w)>2])
    fd_reduced.append(new_string)
...
# remove digits
for i in range(0, len(fd)):
    fd_re.append( re.sub("^\d+\s|\s\d+\s|\s\d+$", "", fd_reduced[i]))
...
# LDA
lda = LatentDirichletAllocation(n_components=20, max_iter = 50, learning_method='online', learning_offset=50., random_state=0)
...
# transform
docs_topics = lda.transform(data_tokens)
...
def get_doc_top_topic(doc_idx, docs_topics):
    return docs_topics[doc_idx].argmax()


# In[150]:


get_doc_top_topic(0, docs_topics)
...
# --------------------------------------------------------------
# Average Term Stability between two models.
# --------------------------------------------------------------
class AverageTermStability:
    def evaluate(self, model_1_term_rankings, model_2_term_rankings):
        if(len(model_1_term_rankings) != len(model_2_term_rankings)):
            raise Exception('Term rankings for model 1 and 2 should be of same length')
        metric = AverageJaccard()
        matcher = RankingSetAgreement(metric)
        score = matcher.similarity(model_1_term_rankings, model_2_term_rankings)
        print(score)
...
def run_lda_return_terms(no_topics, max_iter, tokens, feature_names, no_top_words):
    lda = LatentDirichletAllocation(n_components=no_topics, max_iter = max_iter, learning_method='online', learning_offset=50., random_state=0)
    lda.fit(data_tokens)
    topics_terms = [];
    for topic_idx, topic in enumerate(lda.components_):
        topics_terms.append([feature_names[i] for i in topic.argsort()[:-no_top_words - 1:-1]])
...
# Run LDA models - the output of LDA should be top 10 terms for 20 topics.
def run_lda_iterations(iter_count, no_top_words):
    lda_outputs = []
    for i in range(0, iter_count):
        lda_outputs.append(run_lda_return_terms(20, 10, data_tokens, feature_names, no_top_words))
...
# Run LDA 10 times and evaluate average term stability for 10 runs.
def run_lda(no_top_words):
    lda_outputs = run_lda_iterations(10, no_top_words)
    r = len(lda_outputs)

    all_scores= []

    ats = AverageTermStability()

    for i in range(r):
        for j in range(i+1, r):
            score = ats.evaluate(lda_outputs[i], lda_outputs[j])
            all_scores.append(score)

    # Display the results
    all_scores_array = np.array(all_scores)
    mean = all_scores_array.mean()
    print("Mean stability score: " + str(mean))
    return mean
...
mean_stab = []
mean_stab.append(run_lda(10))


# In[130]:


mean_stab.append(run_lda(30))
mean_stab.append(run_lda(50))
...
# map the LDA topics to true topics
map_topic = {0:2, 1:21, 2:17, 3:1, 4:21, 5:21, 6:4, 7:21, 8:21, 9:18, 10:0, 11:5, 12:15, 13:21, 14:7, 15:21, 16:8, 17:11, 18:10, 19:14}


# In[102]:


# Evaluation of LDA, confusion matrix
import numpy as np
from sklearn.metrics import precision_recall_fscore_support

# get the true labels from dataset.
y_true = np.append(d1.target, d2.target)

# get the predicted labels for all dataset by running the NFM model and picking the topic with highest probabilty distribution.
y_pred = []
for doc_idx in range(0, len(fd)):
    top_topic = get_doc_top_topic(doc_idx, docs_topics)
    y_pred.append(map_topic[top_topic])
...
labels_to_include = [1, 17, 5, 4, 15, 0, 7, 8, 11, 10, 14]


# In[114]:


len(labels_to_include)
...
def cal_eval(label_idx):
    return precision_recall_fscore_support(y_true, y_pred, labels=[label_idx])
...
for i in labels_to_include:
    evals = cal_eval(i)
    print("Evaluation for topic " + str(i))
    print(evals)
