...
nmf = NMF(n_components=no_topics, max_iter=200, init='nndsvd')
...
def get_document_topic(doc_index, W, top_topics_count):
    array = np.array(W[doc_index,:])
    top_topics = np.argsort(array)[::-1]
    top_topics = top_topics[0:top_topics_count]
...
# Runs the NMF model and return an array (length = 20), each item itself an array of top 10 terms.
def run_nmf_return_terms(no_topics, max_iter, tfidf, feature_names, no_top_words):
    nmf_model = NMF(n_components=no_topics, max_iter=max_iter).fit(tfidf)
    topics_terms = [];
...
# Run NMF models - the output of NMF should be top 10 terms for 20 topics.
def run_nmf_iterations(iter_count, no_top_words):
    nmf_outputs = []
    for i in range(0, iter_count):
        nmf_outputs.append(run_nmf_return_terms(20, 200, tfidf, tfidf_feature_names, no_top_words))
...
# Run NMF 10 times and evaluate average term stability for 10 runs.
def runnmf(no_top_words):
    nmf_outputs = run_nmf_iterations(10, no_top_words)
    r = len(nmf_outputs)
    ...
    for i in range(r):
        for j in range(i+1, r):
            score = ats.evaluate(nmf_outputs[i], nmf_outputs[j])
...
mean_stab = []
mean_stab.append(runnmf(30))
mean_stab.append(runnmf(40))
mean_stab.append(runnmf(50))


# In[95]:


tt = get_document_topic(0, W, 1)
tt[0]
...
# Manual mapping of actual topics to NMF topics
map_topic = {0:19, 1:4, 2:15, 3:3, 4:11, 5:21, 6:10, 7:6, 8:21, 9:9, 10:21, 11:2, 12:7, 13:21, 14:14, 15:16, 16:17, 17:1, 18:21, 19:5}
...
y_true = np.append(df1.target, df2.target)
...
y_pred = []
for doc_idx in range(0, len(df_data)):
    top_topic = get_document_topic(doc_idx, W, 1)
    y_pred.append(map_topic[top_topic[0]])
...
labels_to_include = [19, 4, 15, 3, 11, 10, 6, 9, 1, 2, 7, 14, 16, 17, 5]
